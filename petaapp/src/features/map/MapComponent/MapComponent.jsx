...
export default function MapComponent({ activeDataType = 'sawah', zoomToFeature }) {
  ...
  const [isLegendMinimized, setIsLegendMinimized] = useState(() => window.innerWidth <= 1024);
  ...
  const RIVER_COLORS = useMemo(() => {
    const colors = {};
    const palette = ['#2563eb', '#06b6d4', '#0ea5e9', '#1d4ed8', '#0891b2', '#3b82f6', '#1e40af'];
  ...
  const riverStyle = (feature) => {
    ...
    return { color: RIVER_COLORS[name] || '#2563eb', weight: 6, opacity: 1.0 };
  };
